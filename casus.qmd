...
Neem als voorbeeld het Strategy Maps model van Kaplan en Norton. Op het hoogste niveau biedt dit model een raamwerk voor het begrijpen van hoe verschillende aspecten van een organisatie samenwerken om waarde te creëren. Maar om dit model echt effectief te gebruiken, moet je het kunnen ontleden in submodellen en concepten.

Laten we enkele componenten van het Strategy Maps model nemen:

1.  **Financieel perspectief**: Dit kan verder worden ontleed en gerelateerd aan financiële modellen, zoals het [DuPont-analysemodel](https://youtu.be/hHuItcTJJcs?si=a3h3jzGTQECB_jgT){target="_blank"}, wat helpt bij het begrijpen van ROI via aspecten als winstmarge en vermogensomzetsnelheid.
...
3.  **Interne processen**: Dit kan worden verbonden met operationele modellen, zoals het Total Quality Management (TQM) voor het verbeteren van de kwaliteit van processen en producten.

4.  **Leer & Groei**: Hier kunnen we concepten als de Growth Mindset van Carol Dweck en modellen voor personeelsontwikkeling integreren.
...
Hieronder volgt een verkenning van een aantal modellen die je kunnen helpen bij de casus. Het Strategy Maps model wordt hierbij als kapstop gebruikt. Hieraan worden andere modellen en concepten opgehangen en de relaties worden besproken.
